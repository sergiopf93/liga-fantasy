...
def generate_html_report(report: dict) -> str:
    ...
    for a in dec.get("actions", []):
        ...
        # Generar HTML usando concatenación para evitar problemas con llaves CSS
    css = """
    ...
    for r in pat.get("rivals", []):
        ...
        pat_rows += (
            f'<tr style="{bold}"><td>{"⭐ " if is_me else ""}{r["manager"]}</td>'
            f'<td>{r.get("first_fmt","")}</td><td>{r.get("last_fmt","")}</td>'
            f'<td style="color:{color}">{("+" if r.get("growth_pct",0)>=0 else "")}{r.get("growth_pct",0)}%</td></tr>'
        )
    ...
    for r in pts.get("rivals", []):
        ...
        pts_rows += (
            f'<tr style="{bold}"><td>{"⭐ " if is_me else ""}{r["manager"]}</td>'
            f'<td>{r.get("first_points","")}</td><td>{r.get("last_points","")}</td>'
            f'<td style="color:#1a73e8">+{r.get("points_gained","")}</td></tr>'
        )
    ...
    for a in dec.get("actions", []):
        color = result_colors.get(a.get("result","unknown"), "#666")
        dec_rows += (
            f'<tr><td>{str(a.get("executed_at",""))[:10]}</td>'
            f'<td>{a.get("action","").upper()}</td>'
            f'<td>{a.get("player_name","")}</td>'
            f'<td>{a.get("amount",0)/1e6:.2f}M€</td>'
            f'<td style="color:{color}">{a.get("result","?")}</td>'
            f'<td style="font-size:11px;color:#666">{a.get("result_detail","")}</td></tr>'
        )

    growth_color = "#2ea043" if s.get("patrimony_growth_pct",0) >= 0 else "#f85149"
    growth_sign = "+" if s.get("patrimony_growth_pct",0) >= 0 else ""
    period_start = pat.get("period_start","")
    period_end = pat.get("period_end","")
    period_days = report.get("period_days",0)

    html = (
        "<!DOCTYPE html>\n<html lang=\"es\">\n<head>\n"
        "<meta charset=\"UTF-8\">\n"
        f"<title>Informe Analista Fantasy RH - {month}</title>\n"
        f"<style>{css}</style>\n</head>\n<body>\n"
        '<div class="prompt-box">\n'
        '<h2>🤖 Instrucciones para ChatGPT</h2>\n'
        '<p style="font-size:12px;color:#666;margin-bottom:8px">Copia este prompt y pégalo en ChatGPT junto con el informe completo:</p>\n'
        f'<div class="prompt-content">{prompt}</div>\n</div>\n'
        f'<h1>📊 Informe Analista Fantasy R.H.</h1>\n'
        f'<div class="period">Período: {period_start} → {period_end} · {period_days} días</div>\n'
        '<h2>Resumen ejecutivo</h2>\n<div class="summary-grid">\n'
        f'<div class="stat"><div class="stat-val" style="color:{growth_color}">{growth_sign}{s.get("patrimony_growth_pct",0)}%</div><div class="stat-label">Crecimiento patrimonio</div></div>\n'
        f'<div class="stat"><div class="stat-val blue">+{s.get("points_gained",0)}</div><div class="stat-label">Puntos ganados</div></div>\n'
        f'<div class="stat"><div class="stat-val">{s.get("position_start","?")}º → {s.get("position_end","?")}º</div><div class="stat-label">Posición en liga</div></div>\n'
        f'<div class="stat"><div class="stat-val">{pat.get("my_first_fmt","")}</div><div class="stat-label">Patrimonio inicial</div></div>\n'
        f'<div class="stat"><div class="stat-val">{pat.get("my_last_fmt","")}</div><div class="stat-label">Patrimonio final</div></div>\n'
        f'<div class="stat"><div class="stat-val green">{s.get("success_rate",0)}%</div><div class="stat-label">Tasa de éxito</div></div>\n'
        '</div>\n'
        '<h2>💰 Evolución patrimonial comparativa</h2>\n'
        '<table><tr><th>Manager</th><th>Valor inicial</th><th>Valor final</th><th>Crecimiento</th></tr>\n'
        f'{pat_rows}</table>\n'
        '<h2>🏆 Evolución de puntos comparativa</h2>\n'
        '<table><tr><th>Manager</th><th>Puntos inicio</th><th>Puntos final</th><th>Ganados período</th></tr>\n'
        f'{pts_rows}</table>\n'
        ...
        f'{dec_rows or "<tr><td colspan=\"6\" style=\"color:#666;text-align:center\">Sin decisiones registradas este período</td></tr>"}'
        '</table>\n</body>\n</html>'
    )
    return html
